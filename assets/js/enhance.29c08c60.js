...
//  }
    ...
    var svgCSS      = getMeta( svgCSSKey ),
        pngCSS      = getMeta( pngCSSKey ),
        fallbackCSS = getMeta( fontCSSKey );
    if( svgCSS && pngCSS && fallbackCSS ){
        grunticon([svgCSS.content, pngCSS.content, fallbackCSS.content]);
    }
...
//  docElem.className += " " + htmlClasses.join(" ");
    ...
    var swipeJS = getMeta( swipeJSKey );
    if( swipeJS ){
        loadJS( swipeJS.content );
    }   
